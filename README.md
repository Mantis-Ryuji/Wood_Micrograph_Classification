...
本タスクでは、同一 family／同一 genus 内での種判別 も求められます。ところが、それらの種は微細構造や組織配列が非常に似通っており、クラス間のマージンは極めて小さい一方で、樹体差・切片条件・染色や照明条件といった要因によるクラス内分散は大きくなります。そのため、実質的には「双子を見分けるより難しい」ほどの微差判定を要する課題となっています（個人的な意見）。
こうした条件下では、広域的な組織構造やパターンを手がかりにした識別が必要です。しかし、汎用的なCNN（例：ResNet）では十分な分離境界を形成できず、太刀打ちできませんでした。
